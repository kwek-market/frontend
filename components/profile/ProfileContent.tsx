...
const ProfileContent = function ({ activeBtn, setActiveBtn }) {
  return (
    <div className="tw-border tw-border-gray-500 tw-border-opacity-50 tw-rounded tw-p-2 md:tw-p-5 tw-relative">
      ...
      {activeBtn === 'My Orders' && <Orders activeBtn={activeBtn} setActiveBtn={setActiveBtn} />}
      {activeBtn === 'Track My Order' && <Track activeBtn={activeBtn} />}
      {activeBtn === 'My Addresses' && <Addresses activeBtn={activeBtn} />}
      {activeBtn === 'Open Order Details' && <OpenOrderDetails setActiveBtn={setActiveBtn} />}
      {activeBtn === 'Closed Order Details' && <ClosedOrderDetails setActiveBtn={setActiveBtn} />}
    </div>
  );
};
