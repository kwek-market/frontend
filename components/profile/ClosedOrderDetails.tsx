...
const getDeliveryStatusProp = status => {
  if (status === OrderDeliveryStatus.Delivered) {
    return {
      className: "tw-p-1 tw-text-xs tw-text-white-100 tw-bg-green-500 tw-inline tw-uppercase",
  ...
  if (status === OrderDeliveryStatus.Cancelled) {
    return {
      className:
        "tw-p-1 tw-text-xs tw-text-white-100 tw-bg-gray-kwek200 tw-inline tw-opacity-40 tw-uppercase",
    ...
    <div className='tw-p-2 md:tw-p-5 tw-bg-white-100'>
      ...
      <div className='tw-mt-4'>
        <div className='tw-flex tw-flex-row tw-justify-between tw-border-b tw-border-gray-kwek700'>
          ...
          <span>
            <Badge
              badgeStyle={getDeliveryStatusProp(order.deliveryStatus).className}
              text={getDeliveryStatusProp(order.deliveryStatus).text}
            />
