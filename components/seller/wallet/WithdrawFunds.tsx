...
import useWithdrawFromWallet, { WithdrawFromWalletType } from "@/hooks/useWithdrawFromWallet";
    ...
    <main className='tw-px-5'>
      <div className='tw-rounded-md tw-p-3 tw-flex tw-justify-between tw-border tw-border-gray-kwek700'>
        ...
        <p className='tw-flex-[2] tw-ml-2 tw-mb-0'>
          ...
          <span className='tw-capitalize tw-text-red-kwek100'>{seller.bankName}</span> account,{" "}
          <span className='tw-capitalize tw-text-red-kwek100'>{seller.bankAccountNumber}</span>
        </p>
      </div>
      <div id='form' className='tw-mt-3'>
        <div className=''>
          <label htmlFor='amount' className='tw-mt-3 tw-font-semibold tw-gray-kwek900'>
            How much do you want to withdraw?{" "}
            <span className='tw-text-sm tw-text-gray-kwek800'>(minumum NGN 2,000.00)</span>
          </label>
          <Input
            id='amount'
            type='text'
            ...
            onChange={e => setWithdrawFunds({ ...withdrawFunds, amount: e.target.value })}
          />
        </div>
        <div className='tw-mt-3'>
          <label htmlFor='password' className='tw-my-3 tw-font-semibold tw-gray-kwek900'>
            Enter your password
          </label>
          <Input
            id='password'
            type={eye ? "text" : "password"}
            addonAfter={<i className={`fas ${showEye}`} onClick={() => setEye(!eye)} />}
            placeholder='******'
            className='tw-rounded-md'
