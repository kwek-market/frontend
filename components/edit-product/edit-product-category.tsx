...
import { EditProductProps, SubCategoriesType, UploadProductProps } from "@/interfaces/commonTypes";
      ...
      <div className='tw-grid tw-grid-cols-1 md:tw-grid-cols-2 tw-gap-3 tw-mt-3'>
        <label className='tw-text-base tw-font-medium tw-capitalize'>
          ...
          <select
            ...
            required
          >
            <option defaultChecked value={product?.subcategory?.id}>
...
async function fetchCategoryList(search: string, token?: string): Promise<any[]> {
  try {
    const result = await userFetcher(GET_CATEGORIES, { search, token, page: 1 });
...
{
  /* <SearchSelectInput
            fetchOptions={search => fetchCategoryList(search)}
