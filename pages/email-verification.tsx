...
const Emailverification = function () {
  ...
  useEffect(() => {
    if (account.email === "") {
      ...
      router.push("/create-account", { query: { next_page: router?.query?.next_page } });
    }
  }, []);
  ...
  return (
    <AuthLayout id='emailVerification' withBanner={false}>
      <div className='tw-flex tw-h-[80vh] tw-flex-col tw-justify-center tw-items-center tw-p-2 md:tw-p-0'>
        ...
        <p className='tw-text-black-stock tw-text-sm md:tw-text-base'>
          Please confirm that <strong>{account.email} </strong>is your email address by clicking on
          the link sent to your email within the next 24 hours. Also check your spam.
        </p>
        <Button
          buttonStyle='tw-py-2 tw-px-4 tw-bg-red-kwek100 tw-text-white-100 tw-rounded-md tw-capitalize'
          text='sign in'
          cmd={() => {
            router.push("/login", { query: { next_page: router?.query?.next_page } });
