...
const EditCategory = () => {
  ...
  useEffect(() => {
    if (data)
      setFormData({
        name: (data as Record<string, any>).category.name,
        visibility: (data as Record<string, any>).category.visibility,
        icon: (data as Record<string, any>).category.icon,
        parent: (data as Record<string, any>).category?.parent?.name,
        publishDate: (data as Record<string, any>).category.publishDate,
      });
  }, [data]);
          ...
          <div className="tw-border mt-1">
            <FileInputLarge
              ...
              fileList={[
                {
                  name: "category-image",
                  uid: (data as Record<string, any>).category?.id,
                  status: "done",
                  url: (data as Record<string, any>).category?.icon,
                  type: "",
                  size: 900,
                  originFileObj: null,
                },
