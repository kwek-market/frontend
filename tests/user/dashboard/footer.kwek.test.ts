...
test.describe('Footer kwek link Test', () => {
  ...
  footerLinks.forEach(link => {
    test(`should navigate to the correct page when clicking "${link.name}" footer link`, async ({ page }) => {
      const footerLink = page.locator(`footer a`).filter({ hasText: link.name });
