...
test.describe('Admin Dashboard Tests', () => {
    ...
    test('Navigate to customer page ', async ({ page }) => {
      await page.getByRole('link', { name: 'Customers', exact: true }).click();
      await expect(page).toHaveURL('http://localhost:3100/admin/customers');  
    ...
    test('Verify the send email button ', async ({ page }) => { 
      await page.getByRole('link', { name: 'Customers', exact: true }).click();
      await expect(page).toHaveURL('http://localhost:3100/admin/customers');  
    ...
    test('Check the presence of tabs', async ({ page }) => {
      await page.getByRole('link', { name: 'Customers', exact: true }).click();
      await expect(page).toHaveURL('http://localhost:3100/admin/customers');  
